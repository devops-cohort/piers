...
@app.route("/edit_card/<str:card>", methods=['GET','POST']) #done HTML
@login_required
...
@app.route("/remove_card/<str:card>", methods=['GET','POST']) #done
@login_required
...
@app.route("/deck/<str:deck_id>", methods=['GET','POST']) #done HTML
@login_required
def deck(deck_id):
...
@app.route("/add_card/<str:deck>", methods=['GET','POST']) #NEED HTML
@login_required
def add_card(deck):
    form = SearchCard()
    results = []
    if form.validate_on_submit:
        query = card_list.query.filter(card_list.card_name.ilike(form.card_search)).all()
        for entry in query:
...
@app.route("/confirm_card/<str:deck>/<str:card_name>", methods=['GET','POST']) #done
@login_required
